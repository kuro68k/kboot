...
__attribute__((__aligned__(4))) const USB_MicrosoftExtendedPropertiesDescriptor msft_extended = {
	...
	.dwPropLength2 = 14 + (6*2) + (17*2),
	.dwType2 = 1,
	.wNameLength2 = 6*2,
	.name2 = L"Label\0",
	.dwDataLength2 = 17*2,
	.data2 = L"XMEGA Bootloader\0",
};
*/
/*
__attribute__((__aligned__(4))) const USB_MicrosoftExtendedPropertiesDescriptor msft_extended = {
...
extern volatile uint8_t	control_out_complete_SIG;

void usb_cb_control_setup(void) {
	if (((usb_setup.bmRequestType & USB_REQTYPE_TYPE_MASK) == USB_REQTYPE_VENDOR) &&
		(usb_setup.bRequest != WCID_REQUEST_ID))	// not MS descriptor request
	{
		control_out_complete_SIG = 0xFF;
		return;
	}
